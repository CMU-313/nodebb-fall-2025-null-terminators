...
describe("Post Visibility", () => {
	...
	before(async () => {
		...
		// Create test users
		authorUid = await user.create({
			username: "visibilityauthor",
			password: "authorpwd",
		});
		restrictedUserUid = await user.create({
			username: "restricteduser",
			password: "restrictedpwd",
		});
		nonMemberUserUid = await user.create({
			username: "nonmember",
			password: "nonmemberpwd",
		});
		multiGroupUserUid = await user.create({
			username: "multigroupuser",
			password: "multigroupuserpwd",
		});
	...
	it("should only show restricted post to group members", async () => {
		...
		const filteredForMember = await posts.filterPostsByVisibility(
			postData,
			restrictedUserUid,
		);

		// Filter for non-member
		const filteredForNonMember = await posts.filterPostsByVisibility(
			postData,
			nonMemberUserUid,
		);
		...
		// Assertions
		assert.strictEqual(
			filteredForMember.length,
			1,
			"Group member should see the restricted post",
		);
		assert.strictEqual(
			filteredForNonMember.length,
			0,
			"Non-member should NOT see the restricted post",
		);
	});
	...
	it("should allow post author to see their own restricted posts", async () => {
		...
		const filteredForAuthor = await posts.filterPostsByVisibility(
			postData,
			authorUid,
		);

		assert.strictEqual(
			filteredForAuthor.length,
			1,
			"Author should see their own restricted post",
		);
	});

	it("should allow posts with visibleTo: all to be seen by all users", async () => {
		const postData = await posts.getPostsData([allPostPid]);
		const filteredForRestrictedUser = await posts.filterPostsByVisibility(
			postData,
			restrictedUserUid,
		);
		const filteredForNonMember = await posts.filterPostsByVisibility(
		...
		assert.strictEqual(
			filteredForRestrictedUser.length,
			1,
			"Restricted user should see the all post",
		);
		assert.strictEqual(
			filteredForNonMember.length,
			1,
			"Non-member should see the all post",
		);
	});
	...
	it("should not allow guests to see restricted posts", async () => {
		...
		assert.strictEqual(
			filteredForGuest.length,
			0,
			"Guest (uid=0) should NOT see restricted posts",
		);
	});
	...
	it("should allow members of any group to see posts restricted to multiple groups", async () => {
		...
		const filteredForGroup1Member = await posts.filterPostsByVisibility(
			postData,
			restrictedUserUid,
		);
		assert.strictEqual(
			filteredForGroup1Member.length,
			1,
			"Member of first group should see multi-group post",
		);

		// User in both groups should see it
		const filteredForMultiGroupUser = await posts.filterPostsByVisibility(
			postData,
			multiGroupUserUid,
		);
		assert.strictEqual(
			filteredForMultiGroupUser.length,
			1,
			"Member of both groups should see multi-group post",
		);
	});
	...
	it("should not show posts restricted to multiple groups to non-members", async () => {
		...
		assert.strictEqual(
			filteredForNonMember.length,
			0,
			"Non-member of all groups should NOT see multi-group post",
		);
	});
	...
	it("should allow all logged-in users to see posts restricted to registered-users", async () => {
		...
		const filteredForMultiGroup = await posts.filterPostsByVisibility(
			postData,
			multiGroupUserUid,
		);
		...
		assert.strictEqual(
			filteredForRestrictedUser.length,
			1,
			"Logged-in user should see registered-users post",
		);
		assert.strictEqual(
			filteredForNonMember.length,
			1,
			"Logged-in non-member should see registered-users post",
		);
		assert.strictEqual(
			filteredForMultiGroup.length,
			1,
			"Logged-in multi-group user should see registered-users post",
		);
	});
	...
	it("should not allow guests to see posts restricted to registered-users", async () => {
		...
		assert.strictEqual(
			filteredForGuest.length,
			0,
			"Guest should NOT see registered-users post",
		);
	});
	...
	it("should treat posts with no visibleTo field as public", async () => {
		...
		const filtered = await posts.filterPostsByVisibility(
		...
		assert.strictEqual(
			filtered.length,
			1,
			"Posts without visibleTo should be visible to all",
		);
	});
	...
	it("should treat posts with undefined visibleTo as public", async () => {
		...
		const filtered = await posts.filterPostsByVisibility(
			[postWithUndefinedVisibility],
			nonMemberUserUid,
		);
		assert.strictEqual(
			filtered.length,
			1,
			"Posts with undefined visibleTo should be visible to all",
		);
	});
	...
	it("should handle null/undefined posts in array", async () => {
		...
		assert.ok(
			filtered.length >= 0,
			"Should handle null/undefined posts without crashing",
		);
	});

	it("should treat posts with invalid JSON visibleTo as public", async () => {
		const postWithInvalidJSON = {
			pid: 997,
			content: "test",
			visibleTo: "invalid-json-{",
		};
		const filtered = await posts.filterPostsByVisibility(
			[postWithInvalidJSON],
			nonMemberUserUid,
		...
		assert.strictEqual(
			filtered.length,
			1,
			"Posts with unparseable visibleTo should be treated as public",
		);
	});

	// Batch Filtering Tests
	it("should correctly filter mixed public and restricted posts", async () => {
		const allPosts = await posts.getPostsData([
			restrictedPostPid,
			allPostPid,
			multiGroupPostPid,
			registeredUsersPostPid,
		]);
		...
		assert.strictEqual(
			filteredForNonMember.length,
			2,
			"Non-member should see public and registered-users posts",
		);
		...
		assert.strictEqual(
			filteredForMember.length,
			4,
			"Group member should see all accessible posts",
		);
	...
	it("should handle batch with all restricted posts for non-member", async () => {
		const restrictedPosts = await posts.getPostsData([
			restrictedPostPid,
			multiGroupPostPid,
		]);
		const filtered = await posts.filterPostsByVisibility(
		...
		assert.strictEqual(
			filtered.length,
			0,
			"Non-member should not see any restricted posts",
		);
	});
	...
	it("should handle user with no custom group memberships", async () => {
		...
		assert.strictEqual(
			filtered.length,
			0,
			"User with no custom groups should not see custom group posts",
		);
	});

	it("should allow user in multiple groups to see posts from any of their groups", async () => {
		const postData = await posts.getPostsData([
			restrictedPostPid,
			multiGroupPostPid,
		]);
		...
		assert.strictEqual(
			filtered.length,
			2,
			"User in multiple groups should see posts from any of their groups",
		);
	});
	...
	it("should allow author to see multiple own posts with different restrictions", async () => {
		...
		assert.strictEqual(
			filtered.length,
			3,
			"Author should see all their own posts regardless of restrictions",
		);
	});
	...
	it("should not allow non-author to see restricted post", async () => {
		...
		assert.strictEqual(
			filteredForMultiGroup.length,
			1,
			"Multi-group member should see post",
		);

		// authorUid (different from post author) is NOT in testGroupSlug, so should NOT see it
		const filteredForDifferentAuthor = await posts.filterPostsByVisibility(
			postData,
			authorUid,
		);
		assert.strictEqual(
			filteredForDifferentAuthor.length,
			0,
			"Different author not in group should NOT see post",
		);
	});
	...
	it("should handle visibleTo as JSON string", async () => {
		...
		assert.strictEqual(
			filteredForMember.length,
			1,
			"Member should see post with JSON string visibleTo",
		);
		assert.strictEqual(
			filteredForNonMember.length,
			0,
			"Non-member should not see post with JSON string visibleTo",
		);
	});
	...
	it("should handle visibleTo as array", async () => {
		...
		assert.strictEqual(
			filteredForMember.length,
			1,
			"Member should see post with array visibleTo",
		);
		assert.strictEqual(
			filteredForNonMember.length,
			0,
			"Non-member should not see post with array visibleTo",
		);
	});
	...
	it("should handle empty visibleTo array as public", async () => {
		...
		assert.ok(
			filtered.length >= 0,
			"Empty visibleTo array should be handled gracefully",
		);
	});
	...
	it("should handle visibleTo with mixed case group names", async () => {
		...
		assert.strictEqual(
			filtered.length,
			1,
			"Group name matching should work correctly",
		);
	});
});
