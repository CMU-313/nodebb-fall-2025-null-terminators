...
categoryController.get = async function (req, res, next) {
	...
	if (searchTerm) {
		const searchResults = await topics.searchInCategory(
			searchTerm,
			cid,
			req.uid,
		);
	...
	try {
		await Promise.all(
			(categoryData.topics || []).map(async (t) => {
				if (!t) return;
				// Mask teaser user if teaser post is anonymous
				try {
					if (t.teaser && t.teaser.pid) {
						const row = await Posts.getPostFields(t.teaser.pid, [
							"anonymous",
							"uid",
							"pid",
						]);
						const isAnon =
							row && (row.anonymous === true || row.anonymous === "true");
						if (isAnon) {
							const isOwner =
								req.uid && parseInt(req.uid, 10) === parseInt(row.uid, 10);
					...
					/* console.warn('[anon][category] teaser mask failed', e); */
				}
				...
				try {
					if (t.mainPid && t.user) {
						...
						if (isMainAnon) {
							...
							const isOwner =
								req.uid && parseInt(req.uid, 10) === parseInt(mainOwnerUid, 10);
							const canModerate = await privileges.posts.can(
								"posts:moderate",
								t.mainPid,
								req.uid,
					...
					/* console.warn('[anon][category] header mask failed', e); */
				}
			}),
		);
	} catch (e) {
		/* console.warn('[anon][category] masking failed', e); */
	}
