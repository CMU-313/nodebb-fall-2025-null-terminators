...
async function maskAnonymousIfNeeded(caller, post) {
	...
	if (typeof post.anonymous === "undefined") {
		...
		console.warn(
			"[anon] post pid=%s missing `anonymous` field in read payload",
			post.pid,
		);
		return;
	}
...
postsAPI.getSummary = async (caller, { pid }) => {
	...
	if (!postsData[0] || typeof postsData[0].anonymous === "undefined") {
		console.warn(
			"[anon] getSummary pid=%s came back without `anonymous` field; add it to your summary getter",
			pid,
		);
	}
...
postsAPI.getReplies = async (caller, { pid }) => {
	...
	if (postData.length && typeof postData[0].anonymous === "undefined") {
		console.warn(
			"[anon] getReplies parent pid=%s: posts fetched without `anonymous` field; add it to getPostsByPids/post summaries",
			pid,
		);
	}
