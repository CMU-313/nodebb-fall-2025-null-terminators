...
module.exports = function (Topics) {
	...
	Topics.post = async function (data) {
		...
		if (postData.visibleTo !== undefined) {
			postData.visibleTo = await validateVisibleToForReply(
				postData.visibleTo,
				uid,
			);
		}
	...
	Topics.reply = async function (data) {
		...
		const mainPost = await posts.getPostFields(topicData.mainPid, [
			"visibleTo",
		]);
	...
	async function validateVisibleToForReply(visibleTo, uid) {
		...
		const invalidGroups = visibleTo.filter(
			(groupName, index) => groupName !== "all" && !groupsExist[index],
		);

		if (invalidGroups.length > 0) {
			throw new Error(
				`[[error:groups-do-not-exist, ${invalidGroups.join(", ")}]]`,
			);
		}
