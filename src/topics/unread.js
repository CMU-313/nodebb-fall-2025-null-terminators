...
module.exports = function (Topics) {
	...
	async function getTids(params) {
		...
		const userReadTimes = _.mapValues(
			_.keyBy(userTopics.userScores, "value"),
			"score",
		);
	...
	async function getUnreadFollowed(params, userTopics) {
		...
		const unreadFollowed = await db.isSortedSetMembers(
			...
			userTopics.tids_unread.map((t) => t.value),
		);
	...
	async function getUnreadTidList(userTopics, userReadTimes) {
		const unreadTopics = _.unionWith(
			userTopics.categoryTids,
			userTopics.followedTids,
			(a, b) => a.value === b.value,
		)
			.filter(
				(t) =>
					!userTopics.ignoredTids.includes(t.value) &&
					(!userReadTimes[t.value] || t.score > userReadTimes[t.value]),
			)
			.concat(
				userTopics.tids_unread.filter(
					(t) => !userTopics.ignoredTids.includes(t.value),
				),
			)
	...
	async function pushToCategory(topic, topicInfo, userCidState) {
		if (
			topicInfo.isTopicsFollowed[topic.tid] ||
			[
